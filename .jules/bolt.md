...
## 2025-03-05 - Avoid Date Object Instantiation in Loops
**Learning:** In `getPartnerMonthlyReport`, processing large arrays with `new Date(string)` inside a loop is computationally expensive. It causes unnecessary object creation and string parsing overhead for each iteration.
**Action:** Use `Date.parse(string)` to get numeric timestamps directly and compare them against pre-calculated boundary timestamps hoisted outside the loop for O(n) time efficiency.
