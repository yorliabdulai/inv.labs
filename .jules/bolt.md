...
## 2026-04-25 - Optimize String Conversions in Render Array Filters
**Learning:** Found `debouncedSearch.toLowerCase()` running inside `.filter()` callbacks during render for market and mutual funds. Since `.filter()` executes per item (O(n)), the search query was redundantly converted to lowercase hundreds of times. Furthermore, when the search string is empty, `.toLowerCase()` operations were needlessly executing on all properties.
**Action:** Hoist the search string conversion (`.toLowerCase()`) outside the array iteration loop. For empty queries, add short-circuit evaluation (e.g. `query === "" || ...`) to bypass expensive object property conversions entirely.
