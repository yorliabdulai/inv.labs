...
## 2024-05-18 - React/Recharts Tooltip Re-render Anti-pattern
**Learning:** Recharts triggers frequent parent component re-renders during active user interaction (like hovering over data points). If complex nested components (like a `CustomTooltip` function) are defined inline within the parent component's render scope, React will interpret the newly redefined function as a distinct reference on every render cycle. This results in the Tooltip component being entirely unmounted and remounted on every tiny mouse movement, causing severe visual jank and input lag, particularly when rendering hundreds of data points.
**Action:** Always extract `CustomTooltip` (and similar custom Recharts renderer functions like `CustomActiveShape`) OUTSIDE of the parent component's scope. When these externalized components need access to variables from the parent component's state or scope, explicitly pass those values to the component as props (e.g., `<Tooltip content={<CustomTooltip chartType={chartType} />} />`). Recharts handles merging its own injected state (`active`, `payload`, `label`) with your provided props correctly via `React.cloneElement`.
