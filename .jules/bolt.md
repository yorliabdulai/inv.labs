...
## 2026-05-29 - O(N log N) Date Instantiation Inside Sort
**Learning:** Found `new Date(str).getTime()` inside `.sort()` callbacks for ISO 8601 date strings. Instantiating `Date` objects within the sort callback is expensive and adds unnecessary O(N log N) execution overhead.
**Action:** Use direct lexicographical string comparison (`a < b ? -1 : (a > b ? 1 : 0)`) for sorting ISO date strings to avoid instantiating `Date` objects. Also, if an array is already sorted and you need the opposite order, use `.slice().reverse()` instead of re-sorting.
