...
## 2026-05-17 - Pre-calculate Boundaries & Parse Dates for Faster Loop Comparisons
**Learning:** Instantiating new `Date()` objects inside loops iterating over large collections creates excessive memory allocations and triggers garbage collection overhead. Implicit type coercions on dates () are also slower than direct integer comparisons.
**Action:** Use `Date.parse()` to fetch raw numeric timestamps and hoist static boundary timestamps outside loops to achieve faster integer comparisons (O(N) memory overhead to O(1)).
