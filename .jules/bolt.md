...
## 2025-05-05 - Avoid Nested Re-evaluations for Cumulative Time-Series
**Learning:** When computing cumulative time-series data from historical arrays (like in `generatePortfolioHistory`), repeatedly traversing the array from scratch up to a timestamp `t` for each point results in O(N*P) complexity. It also forces redundant, expensive `Date` initializations inside the loop.
**Action:** Pre-parse expensive objects like Dates outside the loop, and use a single forward-moving index with a running state to achieve linear O(N+P) time complexity.
