...
## 2025-03-01 - [HIGH] Prevent Numeric Field Overflow in Server RPCs
**Vulnerability:** A generic "Trade execution failed" message was persisting silently for clients even after fixing the primary database transaction logic because `console.error` logs in Next.js Server Actions only render on the server output. Upon deeper investigation, it was discovered that highly volatile stock changes or `0/0` division errors on market data fetches could result in `NaN`, `Infinity`, or extremely high values (e.g. `1000%`) for `changePercent`. When pushed to PostgreSQL's `DECIMAL(5, 2)` column via the `execute_stock_trade` RPC, this caused an unhandled Postgres "numeric field overflow" or "invalid input syntax for type numeric: NaN" error, aborting the RPC.
**Learning:**
1. Next.js Server Actions execute on the server side, meaning `console.error` calls will not surface in the user's browser console. To assist client debugging, specific or sanitized backend error messages must be deliberately returned in the action's payload.
2. Supabase Javascript numbers passed to PostgreSQL RPC `DECIMAL(precision, scale)` parameters must be aggressively sanitized and bounds-checked. PostgreSQL will throw fatal errors for `NaN`, `Infinity`, or values exceeding the maximum digit count.
**Prevention:** Always bounds-check and sanitize (e.g., `isFinite(num)`, `Math.min(max, Math.max(min, num))`) numeric inputs originating from external APIs before passing them as parameters to Supabase RPCs. Explicitly return detailed `error.message` payloads (in a safe manner) from Server Action `catch` blocks when debugging persistent client-side failures.
