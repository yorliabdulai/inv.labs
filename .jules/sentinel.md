...
## 2026-04-25 - Secure CRON endpoint authentication and error handling
**Vulnerability:** CRON endpoints allowed unauthenticated execution in non-production environments and if the CRON_SECRET was undefined, while also leaking raw error messages to the client.
**Learning:** API routes must fail securely by default. Checking `process.env.NODE_ENV === 'production'` or only checking the secret when it exists leaves the endpoints unprotected. Returning `error.message` directly leaks internal system states.
**Prevention:** Always enforce authentication strictly (`!cronSecret || authHeader !== ...`) in all environments and return generic error messages to clients while logging detailed errors server-side.
