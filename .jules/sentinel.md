...
## 2026-05-25 - Parameter Tampering in Financial Transactions
**Vulnerability:** Client-provided `price` was used to calculate total trade costs without server-side verification, allowing malicious users to execute trades at $0 or arbitrary prices.
**Learning:** Next.js Server Actions directly receive unstructured parameters from the client. Financial calculations must treat all inputs as untrusted, regardless of client-side validation.
**Prevention:** Always re-fetch authoritative financial data (like live stock prices) server-side inside the Server Action before executing transactions.
