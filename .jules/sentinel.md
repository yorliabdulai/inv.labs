...
## 2026-05-03 - CRON Route Authentication Bypass
**Vulnerability:** CRON API routes were vulnerable to unauthorized execution because they skipped authentication outside of production environments or failed open when CRON_SECRET was undefined.
**Learning:** Conditional checks like 'if (cronSecret && authHeader !== ...)' fail open when the environment variable is missing, leaving endpoints completely unprotected by default.
**Prevention:** Always enforce authentication in all environments. Check for the existence of the secret and fail closed: 'if (!cronSecret || authHeader !== `Bearer ${cronSecret}`)'.
