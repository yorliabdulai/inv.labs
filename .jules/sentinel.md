...
## 2026-03-09 - [HIGH] CompleteChallengeForUser IDOR Vulnerability\n**Vulnerability:** The `completeChallengeForUser` server action in `src/app/actions/challenges.ts` accepted a client-provided `userId` parameter, allowing an Insecure Direct Object Reference (IDOR) where users could complete challenges for other users.\n**Learning:** Like other Server Actions, those that manipulate user state must never trust client-provided `userId` fields. They act as public API endpoints.\n**Prevention:** Consistently use `supabase.auth.getUser()` within all user-specific Server Actions to authenticate the user and completely ignore any `userId` parameters provided by the client.
