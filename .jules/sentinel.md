...
## 2026-04-22 - [CRITICAL] CRON Secret Authentication Bypass
**Vulnerability:** CRON API endpoints allowed bypass if `CRON_SECRET` was undefined (treating `Bearer undefined` as valid) or selectively bypassed authentication in non-production environments.
**Learning:** Checking `if (cronSecret && authHeader !== ...)` creates an implicit fail-open bypass when the environment variable is missing. Furthermore, environment-conditional authentication checks (`NODE_ENV === 'production'`) expose sensitive operational endpoints in lower environments.
**Prevention:** CRON routes must always fail securely. Implement a strict fail-closed check: `if (!cronSecret || authHeader !== \`Bearer ${cronSecret}\`)` and enforce it uniformly across all environments.
