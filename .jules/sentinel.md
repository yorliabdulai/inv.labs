...
## 2024-05-24 - Fix IDOR in Challenge Server Actions
**Vulnerability:** IDOR in exported Server Actions `syncChallengeXP` and `completeChallengeForUser` via a raw `userId` parameter and `getServiceClient()`.
**Learning:** Exporting functions in a Next.js `"use server"` file exposes them as endpoints. Relying on client-provided IDs while bypassing RLS creates IDOR vulnerabilities.
**Prevention:** Always authenticate users inside server actions using `supabase.auth.getUser()` rather than accepting identity parameters from the client.
