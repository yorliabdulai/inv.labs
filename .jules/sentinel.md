...
## 2024-05-25 - [MEDIUM] Server Action Error Disclosure via catch blocks
**Vulnerability:** Directly throwing or returning backend `error.message` inside the catch block of Server Actions (e.g., `challenges.ts`, `orders.ts`) to the client UI exposes sensitive information.
**Learning:** In Next.js App Router, whatever is returned or thrown from a Server Action is exposed to the client. Throwing raw database or external API errors can leak internal infrastructure details or query syntax. Returning `error.message` directly in an object has the same effect.
**Prevention:** Catch errors gracefully in Server Actions, log the raw error securely on the server using `console.error`, and return a static, generic, user-friendly message (e.g., 'Failed to cancel order. Please try again.') to the client.
