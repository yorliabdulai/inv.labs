...
## 2024-05-24 - Server Action IDOR and Parameter Tampering Addendum
**Vulnerability:** The `executeStockTrade` server action trusted the client's provided `price` and `changePercent` parameters for financial calculations.
**Learning:** Next.js Server Actions act as open API endpoints. Accepting sensitive calculation parameters from the client without server-side re-verification creates critical security bypasses (Parameter Tampering).
**Prevention:** Always fetch the true values (e.g., real-time market price) securely on the server-side and override the client-provided inputs before performing any financial calculations or database updates.
