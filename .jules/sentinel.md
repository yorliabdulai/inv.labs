...
## 2026-03-09 - [CRITICAL] Parameter Tampering in Stock Trade Execution
**Vulnerability:** The `executeStockTrade` server action (`src/app/actions/stocks.ts`) blindly trusted the `price`, `name`, `sector`, and `changePercent` parameters passed from the client when executing a trade. A malicious user could intercept the request payload and change the `price` to `0.01` to buy stocks for virtually free, or change the `price` to `1000000` to sell stocks for an artificially inflated profit.
**Learning:** Never trust client-provided data for financial calculations or critical business logic. Server Actions in Next.js act as public API endpoints and their arguments can be tampered with by any authenticated user.
**Prevention:** Always fetch the single source of truth for critical data (like real-time stock prices) securely on the server side using dedicated backend services or APIs (e.g., `fetchStockBySymbol`). Calculate all costs, fees, and totals server-side based on the verified data.
