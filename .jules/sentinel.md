...
## 2026-03-10 - Prevented Exposing Error Details via Stock Trading Server Action
**Vulnerability:** The `executeStockTrade` server action (`src/app/actions/stocks.ts`) was directly returning `error.message` to the client in its catch block on a failed execution. This could expose internal database constraints, timeout errors, or API failure strings to the browser.
**Learning:** Directly passing raw `error.message` strings from try-catch blocks in Server Actions acts as an information disclosure channel. In production, these strings should only be logged internally, not rendered to users.
**Prevention:** Always log the raw `error.message` securely on the server (e.g., using `console.error`) and return a generic, static string to the client like "Trade execution failed. Please try again." in the action's `return` payload.
