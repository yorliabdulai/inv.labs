...
## 2024-05-24 - Parameter Tampering in Stock Trading
**Vulnerability:** Client-provided financial data (e.g., price and changePercent) were trusted and used directly to calculate transaction costs and update the database, leading to potential parameter tampering / IDOR.
**Learning:** The application mistakenly trusted client-side state for price and skipped server-side verification, likely because it is a simulator and fetching live data was perceived as unnecessary server-side.
**Prevention:** Always re-fetch authoritative live market data server-side before executing financial transactions, treating all client-provided financial inputs as untrusted.
