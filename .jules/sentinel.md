...
## 2026-03-27 - CSS Injection in Next.js dynamically injected styles
**Vulnerability:** A cross-site scripting (XSS) and CSS injection vulnerability existed in `src/components/ui/chart.tsx` where dynamic user-controlled color values and identifiers were interpolated directly into a `<style dangerouslySetInnerHTML>` block without escaping or validation.
**Learning:** Even when using popular UI library patterns (like shadcn/ui charts), directly interpolating variables into a raw HTML style tag creates a direct injection vector if an attacker controls the chart configuration (e.g., passing `#fff</style><script>alert(1)</script>` as a color).
**Prevention:** Always validate or sanitize dynamic values injected into `<style>` tags. For CSS variables and identifiers, enforce strict alphanumeric formatting. For CSS values, explicitly reject characters that can break out of a CSS declaration or HTML tag (like `;`, `{`, `}`, `<`, `>`, quotes, and newlines).
