...
## 2026-05-22 - Prevent parameter tampering in server actions
**Vulnerability:** Stock trading action trusted client-provided price, changePercent, and metadata to execute trades and calculate fees, allowing malicious users to execute trades at forged prices.
**Learning:** Next.js Server Actions execute code safely on the server, but their arguments come directly from the client. Any financial calculation depending on user inputs can be bypassed via parameter tampering.
**Prevention:** Always treat client-provided parameters (especially prices and monetary values) as untrusted. Re-fetch authoritative live market data server-side and use secure values for all sub-totals and fee calculations.
