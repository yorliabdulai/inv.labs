...
## 2026-04-17 - Fail-Open Authentication in CRON Routes
**Vulnerability:** CRON API endpoints (`process-limit-orders`, `update-mutual-funds`, `re-engagement`) checked authentication conditionally with `if (cronSecret && authHeader !== ...)`, or restricted checks to production only. If the `CRON_SECRET` environment variable was accidentally undefined (or the app was running outside 'production'), the routes failed open, allowing unauthenticated attackers to trigger critical jobs.
**Learning:** Short-circuit authentication checks that depend on the presence of a secret can lead to severe fail-open vulnerabilities if configuration degrades. Default access must always be denied.
**Prevention:** CRON routes must always fail closed. Enforce authentication uniformly across all environments, and explicitly reject access (`if (!cronSecret || ... )`) if the required secret is missing from the environment.
