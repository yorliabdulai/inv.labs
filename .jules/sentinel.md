...
**Vulnerability:** The `createChallenge` server action used `Math.random().toString(36).substring(2, 10)` to generate challenge invite codes. `Math.random()` is not cryptographically secure and produces predictable outputs.
**Learning:** `Math.random()` should never be used for security-sensitive tokens, passwords, or invite codes, as it can be easily predicted, allowing attackers to guess invite codes and join arbitrary private challenges.
**Prevention:** Always use `crypto.getRandomValues()` along with rejection sampling (to avoid modulo bias) for generating secure alphanumeric tokens.
