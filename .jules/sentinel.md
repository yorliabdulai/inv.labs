...
## 2026-04-26 - CRON Authentication Bypass
**Vulnerability:** CRON API routes were either only protected in production environments or allowed access if the `CRON_SECRET` was undefined.
**Learning:** Relying on `NODE_ENV === 'production'` or checking if `cronSecret` is truthy before validating it leaves endpoints fully exposed in other environments or if the environment variable is misconfigured.
**Prevention:** CRON routes must unconditionally enforce authentication by checking `if (!cronSecret || authHeader !== \`Bearer ${cronSecret}\`)` to ensure fail-secure behavior.
