...
## 2026-05-18 - Insecure Randomness in Security-Sensitive Tokens
**Vulnerability:** The `createChallenge` server action generated invite codes using `Math.random().toString(36)`, which is cryptographically insecure and predictable.
**Learning:** For any security-sensitive token (like invite codes, password reset tokens, session IDs), using `Math.random()` opens the application up to token prediction attacks. An attacker could potentially predict or brute-force valid invite codes.
**Prevention:** Always use cryptographically secure random number generators (CSPRNG) like `crypto.randomBytes()` or `crypto.randomUUID()` via the Node.js `crypto` module for generating tokens or secrets. Reserve `Math.random()` for non-security tasks.
