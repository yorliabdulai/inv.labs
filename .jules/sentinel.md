...
## 2026-03-09 - Parameter Tampering in executeStockTrade (Client Payload Trust)
**Vulnerability:** The `executeStockTrade` Server Action accepted critical financial variables (`price`, `changePercent`, `name`, `sector`) directly from the client. While the UI fetched accurate data, a malicious actor could intercept the API request and submit a manipulated payload (e.g., `price: 0.01` or `price: 9999`) to manipulate their cash balance and portfolio value.
**Learning:** Never trust pricing or metadata related to financial transactions submitted by the client. Client-side data is useful for display, but execution logic must independently re-verify all state.
**Prevention:** In Server Actions, ignore client-provided prices. Instead, use the client-provided `symbol` identifier to securely fetch authoritative market data server-side (e.g., via `fetchStockBySymbol`) and calculate all resulting costs and database values using the trusted data. Ensure you gracefully handle the edge case where the server-side fetch returns `null` or `undefined` for a malicious or invalid symbol.
