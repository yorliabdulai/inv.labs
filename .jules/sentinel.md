...
## 2026-03-09 - Client-Side Price and Metadata Tampering in Stock Trading
**Vulnerability:** The `executeStockTrade` server action blindly trusted client-provided `price`, `name`, `sector`, and `changePercent` parameters. This parameter tampering allowed users to manipulate stock prices and avoid trading fees, while potentially causing PostgreSQL numeric overflow crashes if massive invalid values were provided.
**Learning:** Next.js Server Actions act as open API endpoints. Accepting unverified calculation inputs or external reference data (like stock price and metadata) from the client creates severe parameter tampering vulnerabilities.
**Prevention:** Always fetch external data (e.g., real-time market stock data) securely on the server-side, bypassing client provided inputs entirely. Additionally, sanitize and perform robust bounds-checking for the fetched dynamic data (e.g. check for `NaN`, `Infinity`, `<= 0`) to prevent PostgreSQL numeric overflow crashes and protect against database errors.
