...
## 2025-03-25 - Parameter Tampering in Financial Transactions
**Vulnerability:** The `executeStockTrade` server action (`src/app/actions/stocks.ts`) trusted client-provided financial metadata like `price`, `name`, `sector`, and `changePercent` inside `TradeParams`. A malicious user could tamper with the payload to execute trades at $0.01 per share instead of the actual market price.
**Learning:** Never trust the client with sensitive data that drives business logic or calculates costs. The UI's market price is purely for display and must be considered untrusted input on the server.
**Prevention:** In financial systems, only accept user identifiers, order intent (buy/sell), and target symbols from the client. Always re-fetch live market data securely on the backend (e.g., `fetchStockBySymbol`) immediately before executing the transaction to prevent manipulation and TOCTOU vulnerabilities.
