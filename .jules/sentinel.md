...
## 2024-04-06 - Parameter Tampering in Trade Action
**Vulnerability:** Client-provided `price`, `name`, `sector`, and `changePercent` were blindly trusted in `executeStockTrade`.
**Learning:** Next.js Server Actions act as open API endpoints, meaning a malicious user could intercept the request and execute trades at $0.01 instead of the market price.
**Prevention:** Always refetch live pricing and metadata directly on the server-side (`fetchStockBySymbol`) based only on the stock `symbol` to compute total costs, and implement null/undefined checks on the fetched results before execution.
