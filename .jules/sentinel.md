...
## 2024-10-25 - CRON Endpoint Environment Bypass
**Vulnerability:** The CRON endpoint `process-limit-orders` used a check `process.env.NODE_ENV === 'production'` to conditionally enforce the `CRON_SECRET` check. This meant in any other environment (development, staging), the endpoint was unprotected and open to unauthenticated execution.
**Learning:** Security checks should be enforced uniformly across all environments to prevent accidental exposure or bypassing in non-production environments.
**Prevention:** Remove environment conditional checks around authentication logic. Enforce the secret check everywhere, explicitly failing if the secret is missing.
