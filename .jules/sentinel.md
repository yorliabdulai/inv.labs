...
## 2024-05-24 - Parameter Tampering in Financial Transactions
**Vulnerability:** Client-provided financial data (price and changePercent) was directly trusted and used in the `executeStockTrade` Server Action to calculate transaction costs.
**Learning:** In Next.js Server Actions, accepting parameters for cost calculations creates a critical parameter tampering (IDOR/Insecure Direct Object Reference equivalent) vulnerability where clients can buy stocks at arbitrary prices.
**Prevention:** Always treat client-provided financial parameters as untrusted. Re-fetch authoritative live market data server-side and recalculate all sub-totals and fees securely before processing transactions.
