...
## 2026-03-10 - Server Error Leakage in Cron API Routes
**Vulnerability:** Several Next.js API route handlers triggered by cron jobs (`process-limit-orders`, `update-mutual-funds`, `re-engagement`) were catching exceptions and directly returning the raw `error.message` in the JSON response with a 500 status code.
**Learning:** Returning unhandled or raw backend exceptions (like database errors or upstream API failures) via `NextResponse.json({ error: error.message })` exposes internal infrastructure and state to the caller, even in cron endpoints.
**Prevention:** In API route handlers and server actions, always log the full error details securely on the server-side (e.g., via `console.error`) and return a generic, static message to the client/caller (e.g., "An unexpected error occurred while processing the request.").
