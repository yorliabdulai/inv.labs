...
## 2026-04-23 - Fail-Open API Route Protection
**Vulnerability:** CRON API routes used conditionals like `if (cronSecret && authHeader !== ...)` and `if (process.env.NODE_ENV === 'production' && ...)`, which caused the routes to fail-open (bypass authentication entirely) if the `CRON_SECRET` was undefined or the app was running in a non-production environment.
**Learning:** Relying on the existence of a secret to trigger authorization checks creates a critical fail-open vulnerability if environment variables are misconfigured or drop out.
**Prevention:** Always use fail-closed logic: explicitly require the secret to exist and match (`if (!cronSecret || authHeader !== \`Bearer ${cronSecret}\`)`), regardless of the deployment environment.
