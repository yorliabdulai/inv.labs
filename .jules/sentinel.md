...
## 2026-04-16 - Predictable PRNG for Invite Tokens
**Vulnerability:** The `createChallenge` server action used `Math.random()` to generate challenge invite codes. This is cryptographically insecure and predictable, allowing attackers to potentially guess active invite codes and join challenges unauthorized.
**Learning:** Security-sensitive tokens (like invite codes, session IDs, or password reset tokens) must never be generated using predictable pseudorandom number generators like `Math.random()`. Node.js provides secure alternatives via the `crypto` module.
**Prevention:** Always use cryptographically secure random number generators (CSPRNG) like `crypto.randomBytes` or `crypto.randomInt` for any security or access-control related token generation.
