...
## 2026-03-10 - Fix IDOR in Challenge Server Actions
**Vulnerability:** Server Actions like `syncChallengeXP` and `completeChallengeForUser` relied on a client-provided `userId` combined with a service role client (`getServiceClient()`), creating an Insecure Direct Object Reference (IDOR) vulnerability.
**Learning:** In Next.js Server Actions, treating client-provided identifiers as trusted boundaries when modifying data is dangerous, especially when bypassing Row Level Security (RLS) via service clients.
**Prevention:** Always authenticate the caller directly within the Server Action using `supabase.auth.getUser()` and rely on the securely derived `user.id` instead of arbitrary parameters.
