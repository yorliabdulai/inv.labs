...
## 2024-05-27 - CSS Injection Vulnerability via dangerouslySetInnerHTML
**Vulnerability:** The ChartStyle component in `src/components/ui/chart.tsx` dynamically constructed a `<style>` block using `dangerouslySetInnerHTML` with unsanitized keys, ids, and colors from the user configuration. This allowed potential Cross-Site Scripting (XSS) via CSS injection if a user manipulated their custom themes or color settings.
**Learning:** Even internal styling utilities can become vectors for injection attacks if they blindly interpolate variable strings into raw HTML output like `<style>` tags, especially in a generic charting component where "colors" could be pulled from dynamic data sources.
**Prevention:** Always validate and sanitize values used within `dangerouslySetInnerHTML`. For CSS blocks, restrict identifier names using alphanumeric character subsets and validate color formats using strict regexes or boundary checks that reject HTML/CSS breakout characters (like `{`, `}`, `<`, `>`, `"`, `'`).
