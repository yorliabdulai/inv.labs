...
## 2024-05-07 - Insecure CRON Secret Authorization Pattern
**Vulnerability:** CRON endpoints only required a secret token in production or only validated the secret if the `CRON_SECRET` environment variable was set (fail-open).
**Learning:** Fail-open conditional checks like `if (cronSecret && ...)` or conditionally checking environments `if (process.env.NODE_ENV === 'production' && ...)` bypass security protections entirely if the secret is unconfigured or in non-production environments.
**Prevention:** Use a fail-closed pattern for all environments: `if (!cronSecret || authHeader !== \`Bearer ${cronSecret}\`)`. Endpoints should be protected by default and fail securely when misconfigured.
