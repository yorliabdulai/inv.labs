...
## 2024-05-07 - Insecure Randomness in Invite Codes
**Vulnerability:** Found `Math.random()` used to generate `invite_code` tokens in `src/app/actions/challenges.ts`.
**Learning:** Using `Math.random()` for authorization tokens or invite codes makes them predictable and vulnerable to guessing or brute-force attacks.
**Prevention:** Always use cryptographically secure random number generators (e.g., `crypto.randomBytes` in Node.js) for any sensitive tokens or identifiers.
