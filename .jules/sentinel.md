...
## 2025-02-14 - Prevent Parameter Tampering in Financial Transactions
**Vulnerability:** The `executeStockTrade` server action relied on client-provided parameters (such as `price`, `name`, `sector`, and `changePercent`) for execution and cost calculation.
**Learning:** This could allow a malicious actor to perform parameter tampering by sending arbitrary prices to execute trades at their desired rate instead of the actual market price.
**Prevention:** Always fetch actual market data securely on the server side (e.g., using `fetchStockBySymbol`) and only accept identifiers (e.g., `symbol`) and user intents (`type`, `quantity`) from the client.
