...
## 2024-05-25 - Server Action Parameter Tampering (Stock Trading)
**Vulnerability:** The `executeStockTrade` server action blindly trusted client-provided `price` and `changePercent` parameters. This allowed parameter tampering, where users could manipulate stock prices to buy cheaply or sell high.
**Learning:** Client-provided financial parameters must never be trusted for critical operations. Re-fetch authoritative values securely server-side.
**Prevention:** Fetch data like current stock prices via backend services (e.g., `fetchStockBySymbol`) inside the server action before finalizing trades.
