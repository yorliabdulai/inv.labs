...
## 2026-05-29 - Trusting Client Financial Data in Server Action
**Vulnerability:** Parameter Tampering where client-provided stock price was used to calculate trade subtotal.
**Learning:** Next.js Server Actions receive untrusted input. Financial calculations must use authoritative server-side data.
**Prevention:** Always re-fetch authoritative data (like live stock prices) inside the Server Action and recalculate values securely rather than trusting client parameters.
