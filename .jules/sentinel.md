...
## 2024-03-09 - CSS Injection via dangerouslySetInnerHTML in Recharts
**Vulnerability:** In `src/components/ui/chart.tsx`, user-influenced keys and colors from the Chart configuration were injected directly into a `<style>` tag using `dangerouslySetInnerHTML`.
**Learning:** Using `dangerouslySetInnerHTML` for CSS allows attackers to inject malicious CSS characters like semicolons (`;`) or brackets (`{}`) if the values aren't strictly sanitized. This could allow altering page styling, hiding components, or introducing exfiltration attacks via CSS `url()`.
**Prevention:** Always strictly validate and sanitize CSS keys (e.g., using `/[^a-zA-Z0-9_-]/g`) and colors (blocking characters like `{`, `}`, `;`, `"`, `'`) before interpolating them into `<style>` block content. Created `sanitizeCssIdentifier` and `isValidCssColor` in `src/lib/utils.ts` to accomplish this safely.
