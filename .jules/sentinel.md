...
## 2024-05-24 - Parameter Tampering in Trade Execution
**Vulnerability:** The `executeStockTrade` Server Action trusted client-provided `price`, `name`, `sector`, and `changePercent` values to execute stock trades. An attacker could tamper with these parameters to steal simulator funds.
**Learning:** Never trust client inputs for critical financial calculations, even in a simulator. Always re-fetch authoritative data server-side before execution.
**Prevention:** Implement server-side verification using authoritative data sources immediately prior to transaction execution.
