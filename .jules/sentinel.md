...
## 2024-05-30 - CRON Endpoint Authentication Bypass
**Vulnerability:** CRON endpoints across the application used weak authentication checks like `if (cronSecret && authHeader !== \`Bearer ${cronSecret}\`)` or only enforced auth in production (`NODE_ENV === 'production'`). This meant if the secret wasn't set, or in dev/staging environments, the endpoints were completely open, allowing unauthenticated remote execution of sensitive cron logic.
**Learning:** Conditional authentication checks based on the presence of a secret or the environment open silent vulnerabilities. If a secret is missing (e.g., misconfigured environment variables), the system should fail securely (deny access), not default to open access.
**Prevention:** Always enforce strict validation: `if (!cronSecret || authHeader !== \`Bearer ${cronSecret}\`)` to ensure default-deny behavior if secrets are missing.
