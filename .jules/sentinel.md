...
## 2025-03-05 - Insecure Predictable Challenge Invite Codes
**Vulnerability:** The `createChallenge` server action used `Math.random()` to generate the `inviteCode` for new challenges. `Math.random()` is not a Cryptographically Secure Pseudo-Random Number Generator (CSPRNG), making the generated invite codes predictable and potentially allowing attackers to guess valid codes to join private challenges uninvited.
**Learning:** For security-sensitive tokens (e.g., invite codes, session tokens), `Math.random()` must never be used due to its lack of sufficient entropy and predictability.
**Prevention:** Always use a CSPRNG like `crypto.randomBytes(4).toString('hex').toUpperCase()` via the Node.js `crypto` module for generating secure tokens, and reserve `Math.random()` for non-critical tasks like simulation noise or data shuffling.
