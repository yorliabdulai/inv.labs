...
## 2025-03-20 - Fix IDOR vulnerability in AI Chat API
**Vulnerability:** IDOR in `/api/ato/chat` where any authenticated user could read or append to another user's conversation by providing its ID.
**Learning:** The `conversationId` provided by the client was not verified to belong to the authenticated user before it was used to fetch conversation history and append new messages.
**Prevention:** Always verify ownership of client-provided object identifiers (e.g., `conversationId`) against the authenticated user (`user.id`) via a database query before processing read or write actions, returning a 403 Forbidden response if they do not match.
