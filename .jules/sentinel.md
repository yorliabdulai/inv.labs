...
## 2026-05-12 - CRON Route Authentication Bypass and Fail-Open
**Vulnerability:** CRON API routes (`process-limit-orders`, `update-mutual-funds`, `re-engagement`) were improperly protected. One route disabled protection entirely outside of production (`NODE_ENV === 'production'`). The others failed open if the `CRON_SECRET` environment variable was undefined (`if (cronSecret && ...)`).
**Learning:** Default-open security configurations or relying on environment variable presence for authentication checks can lead to complete security bypasses if misconfigured or running in non-production environments that are still accessible.
**Prevention:** Always fail securely (fail-closed). Ensure sensitive routes are protected in all environments and that missing secrets explicitly block access rather than skipping the verification check.
