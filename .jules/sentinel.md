...
## 2026-06-05 - Prevent Parameter Tampering in Financial Transactions
**Vulnerability:** Client-provided prices (`price`) and metadata (`changePercent`) were trusted in Server Actions during stock trades.
**Learning:** Next.js Server Actions execute on the server but their inputs come directly from the untrusted client, enabling malicious users to execute trades at arbitrary prices (IDOR / Parameter Tampering) by modifying the request payload.
**Prevention:** Never trust client-provided financial data. Always re-fetch authoritative live market data server-side (e.g., `fetchStockBySymbol`) and recalculate sub-totals and fees securely before processing transactions.
