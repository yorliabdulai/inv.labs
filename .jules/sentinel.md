...
## 2026-03-30 - [HIGH] Prevent CSS Injection in Chart Components
**Vulnerability:** The `ChartStyle` component in `src/components/ui/chart.tsx` used `dangerouslySetInnerHTML` to inject dynamic CSS styles based on chart configurations (`config.theme` and `config.color`). If these values were user-controlled, an attacker could inject malicious CSS or escape the `<style>` block to execute Cross-Site Scripting (XSS).
**Learning:** Component libraries often rely on dynamic CSS injection for theming. Any dynamic values interpolated into `<style>` tags or `style` attributes must be strictly validated and sanitized, especially if the configuration source is external or user-generated.
**Prevention:** Always validate and sanitize dynamic CSS values before injection. Use an allowlist approach for CSS properties (e.g., restricting keys to alphanumeric characters) and strictly validate CSS values (e.g., ensuring colors match expected hex, rgb, or var formats) to prevent injection of malicious characters like semicolons, braces, or quotes.
