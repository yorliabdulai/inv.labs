...
## 2026-06-06 - Server Action Parameter Tampering
**Vulnerability:** The `executeStockTrade` Server Action trusted client-provided financial data (`price`, `changePercent`, etc.) to calculate trade costs, allowing for parameter tampering and arbitrary price execution.
**Learning:** Server Actions expose an API endpoint that can be invoked with any payload, not just the ones from the frontend UI. Trusting client data for financial sub-totals leads to IDOR and tampering risks.
**Prevention:** Always re-fetch authoritative live market data server-side using secure endpoints (like `fetchStockBySymbol`) and calculate costs securely on the server before processing transactions.
