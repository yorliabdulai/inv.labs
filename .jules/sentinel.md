...
## 2026-03-10 - [CRITICAL] CSS Injection in Recharts Wrapper
**Vulnerability:** The `ChartStyle` component in `src/components/ui/chart.tsx` injected dynamic CSS via `dangerouslySetInnerHTML` directly using configuration colors, keys, and component IDs without sanitization. This allowed attackers controlling these props to inject malicious CSS characters (like `}`, `<`, `>`, `;`), potentially leading to CSS injection or Cross-Site Scripting (XSS) by breaking out of the `<style>` tag.
**Learning:** Even internal UI components can introduce security vulnerabilities if they accept dynamic configurations and blindly render them inside `<style>` tags via `dangerouslySetInnerHTML`. Relying on `CSS.escape()` is problematic because it's undefined in Node.js (SSR) environments, leading to runtime crashes.
**Prevention:** Always sanitize dynamic values injected into `<style>` tags. Use regex validation to strip unsafe characters from colors (`[{};<>"']`), and use inline regex escaping for identifiers (e.g., `id.replace(/\\/g, "\\\\").replace(/"/g, '\\"').replace(/<\/style>/gi, "")` and `key.replace(/[^a-zA-Z0-9-_\.]/g, "")`) to ensure SSR compatibility without crashing the backend.
