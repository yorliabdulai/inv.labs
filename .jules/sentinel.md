...
## 2026-04-21 - CRON Route Authentication Bypass
**Vulnerability:** CRON API endpoints (`process-limit-orders`, `re-engagement`, `update-mutual-funds`) were bypassing authentication checks. Some only checked in production environments, while others allowed access if `process.env.CRON_SECRET` was undefined.
**Learning:** Security-sensitive API routes like CRON jobs should never have conditional authentication bypasses (e.g. `process.env.NODE_ENV === 'production'`) or fail open if configuration is missing. This risks exposing automated administrative actions to anyone.
**Prevention:** CRON routes must always enforce authentication by verifying the token matches `process.env.CRON_SECRET` and returning 401 Unauthorized if the secret is undefined or the header is mismatched.
