...
## 2026-05-16 - Fix CRON API Authorization Bypass
**Vulnerability:** CRON API routes were vulnerable to unauthenticated access. Some only enforced checks in production (`NODE_ENV === 'production'`), and others skipped authentication entirely if the `CRON_SECRET` environment variable was missing (`if (cronSecret && ...)`).
**Learning:** API routes designed for scheduled tasks must fail closed. Conditional authentication checks based on environment or the presence of a secret can lead to severe unauthorized access, exposing administrative or system operations.
**Prevention:** Always enforce authentication securely in all environments. Require the secret to be defined and strictly compare it (`if (!cronSecret || authHeader !== \`Bearer ${cronSecret}\`)`).
