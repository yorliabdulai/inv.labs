...
## 2026-04-24 - [CRITICAL] Fix CRON Authentication Bypass
**Vulnerability:** CRON API routes were vulnerable to unauthenticated access if the `CRON_SECRET` environment variable was undefined, or if the environment was not explicitly set to production.
**Learning:** Checking `if (cronSecret && ...)` bypasses the entire security check when the secret is missing. Similarly, wrapping auth in `NODE_ENV === 'production'` leaves non-production environments exposed.
**Prevention:** Always enforce strict checking `if (!cronSecret || authHeader !== ...)` across all environments to ensure endpoints are never unprotected by default.
