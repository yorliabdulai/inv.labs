...
## 2026-06-01 - Parameter Tampering in Trade Execution
**Vulnerability:** Client-provided price was used directly in `executeStockTrade` for calculating subtotals and fees, allowing users to manipulate trade execution prices (IDOR / Parameter Tampering).
**Learning:** Even if an app is a simulator or uses gamified data, relying on client-side financial figures breaks integrity and leaderboards.
**Prevention:** Always re-fetch authoritative live market data server-side before executing financial transactions and recalculating costs.
