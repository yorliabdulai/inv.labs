...
## 2026-05-30 - Prevent Parameter Tampering in Financial Transactions
**Vulnerability:** The `executeStockTrade` server action blindly trusted client-provided `price` and `changePercent` parameters to calculate transaction costs and execution logic. This allowed malicious users to manipulate the payload (e.g., set `price` to 0.01) and artificially inflate their portfolio value.
**Learning:** Even in applications described as simulators, all financial calculations (subtotals, fees) must be strictly computed using authoritative, server-retrieved data (e.g., `fetchStockBySymbol`). Relying on the client for anything beyond the intended `symbol` and `quantity` creates an immediate parameter tampering/IDOR risk.
**Prevention:** Always refetch live pricing data server-side immediately before transaction execution. Deprecate client-provided pricing fields in action signatures and treat all client inputs as strictly untrusted.
