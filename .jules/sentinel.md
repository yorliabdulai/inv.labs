...
## 2026-05-02 - Unprotected CRON API Routes via Undefined Secret Bypass
**Vulnerability:** CRON API endpoints allowed bypass if `process.env.CRON_SECRET` was undefined (using `if (cronSecret && authHeader !== ...)` or `NODE_ENV` checks), leaving them fully unauthenticated by default.
**Learning:** Conditional checks on security secrets that skip validation if the secret is missing cause insecure defaults. If an environment variable fails to load, the endpoint becomes publicly accessible.
**Prevention:** Fail closed. Always require the secret to be defined and strictly enforce the authentication match: `if (!cronSecret || authHeader !== ...)`.
