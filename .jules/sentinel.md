...
## 2026-05-17 - Fail-Open Authentication in Cron Endpoints
**Vulnerability:** Cron endpoints failed open by skipping authorization checks if CRON_SECRET was missing from environment variables, or accepted 'Bearer undefined' as a valid token.
**Learning:** Checking for the presence of a secret and conditionally validating allows unauthenticated access if misconfigured. String interpolation of missing variables leads to predictable 'undefined' strings.
**Prevention:** Always mandate secret presence in secure contexts (e.g. production) and explicitly fail closed (500 Server Error) if configuration is missing, rather than bypassing the check.
