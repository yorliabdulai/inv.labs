...
## 2025-03-19 - Fix Insecure Direct Object Reference (IDOR) in Ato Chat API
**Vulnerability:** A critical IDOR vulnerability allowed an authenticated user to append messages to and read history from an AI conversation (`ato_conversations`) belonging to another user simply by guessing or knowing the `conversationId`.
**Learning:** By directly taking the `conversationId` input from the request body and utilizing it in Supabase `.eq("conversation_id", conversationId)` queries without prior authorization validation, the application failed to ensure that the current `user.id` actually owned the referenced conversation.
**Prevention:** Always verify ownership of a direct object reference passed by the client before processing an action on it. This means actively querying the database table (e.g., `ato_conversations`) for the object using the object ID and asserting `object.user_id === user.id`, returning `403 Forbidden` if unauthorized.
