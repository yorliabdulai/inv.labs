...
## 2026-05-18 - [CRITICAL] Parameter Tampering in executeStockTrade
**Vulnerability:** The `executeStockTrade` Server Action blindly trusted client-provided `price`, `name`, `sector`, and `changePercent` parameters for processing stock purchases. It also returned raw `error.stack` details to the client.
**Learning:** Next.js Server Actions are public endpoints; trusting client-calculated financial data creates critical parameter tampering vulnerabilities, and uncaught backend errors can leak stack traces.
**Prevention:** Re-fetch all dynamic financial data (like live stock prices) securely on the server side (e.g., via `fetchStockBySymbol`) and sanitize all error responses sent back to the client.
