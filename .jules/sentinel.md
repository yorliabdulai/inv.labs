...
## 2024-04-27 - Fail-open vulnerability in CRON routes
**Vulnerability:** CRON API routes were checking `if (cronSecret && authHeader !== \`Bearer ${cronSecret}\`)`, meaning if the `CRON_SECRET` environment variable was undefined, the check was completely bypassed and the endpoints failed open, allowing unauthenticated access. Additionally, one route completely disabled authentication outside of production.
**Learning:** Relying on the existence of an environment variable as a condition to enforce authentication can lead to dangerous fail-open scenarios if the variable is misconfigured or missing. Security checks should always fail closed.
**Prevention:** Always enforce authentication unconditionally in all environments. If a required secret is missing, deny access by default using the pattern: `if (!cronSecret || authHeader !== \`Bearer ${cronSecret}\`)`.
