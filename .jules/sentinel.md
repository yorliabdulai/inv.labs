...
## 2026-04-15 - Challenge Actions IDOR Prevention
**Vulnerability:** The `syncChallengeXP` and `completeChallengeForUser` Server Actions in `src/app/actions/challenges.ts` accepted a client-provided `userId` parameter and used the service client to mutate challenge data, allowing any user to maliciously alter another user's progress.
**Learning:** Next.js Server Actions act as open API endpoints. Never trust or accept sensitive identifiers (e.g., `userId`) as client-provided parameters. Always authenticate the caller securely via `supabase.auth.getUser()` directly within the server action to prevent Insecure Direct Object Reference (IDOR) vulnerabilities. Avoid using `getServiceClient()` in exposed actions, as it utilizes the service role key and bypasses Row Level Security (RLS).
**Prevention:** In Server Actions mutating user-specific data, immediately fetch the authenticated user session via `supabase.auth.getUser()`, compare any passed identifiers against the authenticated session ID, and return early (or throw) if they do not match, ensuring operations only execute on behalf of the verified owner.
