...
## 2025-03-06 - Prevent Error Message Leakage in API Routes and Server Actions
**Vulnerability:** Potential information disclosure (CWE-209). API routes (`src/app/api/ato/chat/route.ts`) and server actions (`src/app/actions/stocks.ts`) were returning raw `error.message` strings to the client in generic catch blocks.
**Learning:** Returning unhandled `error.message` properties from backend operations can inadvertently expose database structure, query details, stack paths, or other sensitive internal execution state. Even if specific user-facing errors are thrown intentionally, a generic catch block will still expose unforeseen internal errors.
**Prevention:** Always implement an allowlist for known, safe error messages that can be forwarded to the client. Fall back to a generic, opaque error message (e.g., "An unexpected error occurred") for all other caught exceptions. Log the full, raw error securely on the server-side instead.
