...
## 2026-03-09 - [CRITICAL] Prevent Insecure Direct Object Reference (IDOR) in ATO Chat API
**Vulnerability:** The ATO chat API endpoint (`src/app/api/ato/chat/route.ts`) accepted a `conversationId` parameter from the client and used it to append messages to the conversation without verifying if the authenticated user actually owned that conversation.
**Learning:** Client-provided object identifiers (IDs) in API endpoints can be easily manipulated by an attacker to access or modify resources belonging to other users (IDOR).
**Prevention:** Always explicitly verify ownership of client-provided object identifiers against the authenticated user (`user.id`) in the database before processing read or write actions. For instance, query the database to ensure the `user_id` of the resource matches the current session user. Return a generic `403 Forbidden` for both unauthorized access and non-existent records to prevent resource enumeration.
