...
## 2026-03-10 - Chart CSS Injection and XSS via dangerouslySetInnerHTML
**Vulnerability:** The Chart component (`src/components/ui/chart.tsx`) was dynamically generating CSS inside `<style dangerouslySetInnerHTML={{ __html: ... }}>` without sanitizing the chart `id`, variable keys, or colors. This allowed malicious input passed as chart configuration to break out of the CSS context and inject arbitrary CSS rules or execute Cross-Site Scripting (XSS) via injected `<script>` tags.
**Learning:** When injecting dynamic CSS into `<style>` tags via `dangerouslySetInnerHTML`, simply attempting to remove `</style>` tags or escaping values with `CSS.escape()` is either flawed or breaks Server-Side Rendering (SSR) because `CSS.escape()` is undefined in Node.js.
**Prevention:** Always use strict inline regex allowlisting (e.g., `id.replace(/[^a-zA-Z0-9-_\:]/g, "")`) for CSS identifiers. For CSS colors, implement a rigorous validation function (`isValidCssColor`) to explicitly block characters like `{`, `}`, `;`, `"`, `'`, `<`, and `>` to prevent context breakout and XSS.
