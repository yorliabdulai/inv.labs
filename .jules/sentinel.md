...
## 2024-03-23 - Parameter Tampering in executeStockTrade
**Vulnerability:** The `executeStockTrade` server action blindly trusted client-provided `price`, `name`, `sector`, and `changePercent` parameters. A malicious user could tamper with the `price` parameter to buy or sell stock at any arbitrary value (e.g., GH₵0.01).
**Learning:** Next.js Server Actions act as open API endpoints. Accepting unverified pricing or metadata from the client creates severe parameter tampering vulnerabilities.
**Prevention:** Always fetch authoritative pricing and metadata securely on the server-side within the Server Action using internal APIs (e.g., `fetchStockBySymbol`) before proceeding with execution.
