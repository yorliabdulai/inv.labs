...
## 2024-04-01 - Prevent CSS Injection in Recharts Tooltips
**Vulnerability:** The `dangerouslySetInnerHTML` in `src/components/ui/chart.tsx` uses object properties directly to build CSS in `<style>` blocks without validation, allowing potential CSS injection attacks.
**Learning:** The configuration object used to generate dynamic CSS styles can contain unexpected characters, allowing attackers to inject arbitrary CSS rules or execute XSS if the inputs are not properly sanitized and validated.
**Prevention:** Always use strict sanitization functions like `sanitizeCssIdentifier` for CSS selectors and keys, and validate color values using `isValidCssColor` to aggressively block CSS injection characters like `{`, `}`, `;`, `"`, and `'` before rendering dynamic styles via `dangerouslySetInnerHTML`.
