...
## 2026-06-03 - Prevent Parameter Tampering for Financial Data
**Vulnerability:** Client-provided financial data (e.g., price, changePercent) was implicitly trusted by server actions, allowing parameter tampering to buy assets at arbitrary prices.
**Learning:** Server actions must always fetch authoritative data from a trusted source (like an internal database or market data API) regardless of what UI states dictate.
**Prevention:** Completely remove financial parameters from TS interfaces exposing server actions to prevent their usage, and retrieve required states server-side.
