...
## 2024-05-18 - Missing IDOR Check in ATO Chat Endpoint
**Vulnerability:** In `src/app/api/ato/chat/route.ts`, the endpoint previously accepted a `conversationId` provided by the client without verifying if that conversation actually belonged to the authenticated user.
**Learning:** This allowed users to pass arbitrary conversation IDs, potentially reading and modifying other users' conversations (Insecure Direct Object Reference). Trusting client-provided identifiers directly for database queries without ownership verification is a critical security flaw.
**Prevention:** Always explicitly check ownership (e.g., verifying `conversation.user_id === user.id`) before allowing operations on client-provided IDs. Use a generic `403 Forbidden` message to avoid resource enumeration (leaking whether the ID actually exists).
