...
## 2026-05-13 - Fail-Open Authentication in CRON Routes
**Vulnerability:** CRON API endpoints failed open and bypassed authentication if `process.env.CRON_SECRET` was undefined, or in non-production environments.
**Learning:** The pattern `if (cronSecret && authHeader !== ...)` meant that a missing environment variable disabled security completely rather than blocking access.
**Prevention:** Always use fail-secure defaults by checking `if (!cronSecret || authHeader !== ...)` and never bypass authentication based on `NODE_ENV` for exposed API endpoints.
