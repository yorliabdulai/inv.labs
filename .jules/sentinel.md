...
## 2024-06-10 - CRON Route Authentication Bypass
**Vulnerability:** CRON API routes in the application used conditional or fail-open checks for the `CRON_SECRET` (e.g. `if (process.env.NODE_ENV === 'production')` or `if (cronSecret && ...)`). This allowed unauthenticated execution in non-production environments or if the secret was accidentally left undefined, exposing sensitive recurring tasks.
**Learning:** Security controls like authentication must be consistently applied across all environments and fail securely. If an authentication secret is missing, the system should default to denying access rather than allowing it.
**Prevention:** Enforce authentication universally without environment-specific bypasses. Always use the pattern `if (!cronSecret || authHeader !== \`Bearer ${cronSecret}\`)` to ensure CRON routes remain protected even if environment variables are misconfigured.
