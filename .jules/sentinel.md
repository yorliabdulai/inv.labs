...
## 2024-03-20 - Prevent Parameter Tampering in Financial Transactions
**Vulnerability:** The client-side application was responsible for passing critical stock attributes (`price`, `name`, `sector`, `changePercent`) to the server-side `executeStockTrade` action. Since client input cannot be trusted, a malicious user could intercept the request and inject a tampered price (e.g., GH₵0.01 instead of GH₵100.00), allowing them to artificially inflate their virtual cash balance during a SELL or buy shares for almost nothing.
**Learning:** Financial values, fees, and calculations should always be computed using server-fetched data. The server should only accept identifiers (e.g., `symbol`) and user intents (e.g., `type`, `quantity`) from the client to prevent parameter tampering.
**Prevention:** Refactor Server Actions to act as orchestration layers that fetch the necessary external data (e.g., `fetchStockBySymbol`) independently, validating and orchestrating the database transactions securely on the backend instead of trusting front-end payloads.
