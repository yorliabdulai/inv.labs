...
## 2026-04-28 - Fail-Open Environment Variable Checks
**Vulnerability:** CRON API endpoints allowed unauthenticated access when `CRON_SECRET` was undefined or when not in the production environment.
**Learning:** Implementing `if (secret && ...)` creates a fail-open state where a missing environment variable completely disables authentication rather than enforcing it. Removing `NODE_ENV` checks ensures that even in development or preview environments, sensitive endpoints are consistently protected.
**Prevention:** Always use fail-secure conditional logic (`if (!secret || ...)`) for authentication checks. Never conditionally disable auth based on environment variables unless it's explicitly for a public endpoint.
