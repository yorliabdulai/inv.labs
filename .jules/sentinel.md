...
## 2026-05-09 - CRON Endpoint Authorization Bypass
**Vulnerability:** CRON API endpoints allowed unauthenticated requests if the `CRON_SECRET` environment variable was undefined (using `if (cronSecret && ...)`) or only checked authorization in production (`NODE_ENV === 'production'`). This allowed unauthorized execution of critical cron jobs.
**Learning:** Security checks that conditionally fail open when secrets are missing are severe vulnerabilities. CRON endpoints must always be secured and fail closed in all environments.
**Prevention:** Enforce strict presence checks: `if (!cronSecret || authHeader !== \`Bearer ${cronSecret}\`)` to ensure endpoints always fail securely if a secret isn't provided.
