...
## 2024-05-24 - Server Action Parameter Tampering
**Vulnerability:** The `executeStockTrade` server action blindly trusted client-provided `price`, `name`, `sector`, and `changePercent` parameters. This allowed parameter tampering, enabling a malicious user to manipulate the `price` parameter to buy stocks for pennies or sell them for millions.
**Learning:** Next.js Server Actions act as open API endpoints. Accepting sensitive calculation results or prices from the client without server-side re-verification creates critical security bypasses.
**Prevention:** Always recalculate all financial amounts, fees, and fetch real-time prices securely on the server-side instead of relying on client-side values.
