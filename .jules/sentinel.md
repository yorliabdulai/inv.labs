...
## 2026-04-19 - Fix fail-open authentication in CRON routes
**Vulnerability:** CRON API routes were vulnerable to unauthorized execution due to fail-open authentication checks (e.g. bypassing auth if secret was undefined or not in production).
**Learning:** Checking if a secret exists before validating it leads to unauthenticated access if the environment variable is missing.
**Prevention:** Always enforce strict authentication in all environments, and fail closed if the required secret is not defined.
