...
export async function getPartnerMonthlyReport(partnerId: string, month: number, year: number) {
    ...
    try {
        ...
        // ⚡ Bolt: Pre-calculate boundary timestamps outside the loop to avoid redundant date instantiations
        ...
        for (const ref of referrals) {
            if (ref.activation_status && ref.activation_date) {
                // ⚡ Bolt: Use Date.parse to get numerical timestamps directly
