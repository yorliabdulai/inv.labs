...
export async function getPartnerMonthlyReport(partnerId: string, month: number, year: number) {
    ...
    try {
        ...
        // ⚡ Bolt: Pre-calculate boundary timestamps outside the loop to avoid redundant date operations
        ...
        for (const ref of referrals) {
            if (ref.activation_status && ref.activation_date) {
                // ⚡ Bolt: Use Date.parse() to get primitive timestamps directly instead of instantiating new Date objects inside the O(n) loop
