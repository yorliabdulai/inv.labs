...
export async function executeStockTrade(params: TradeParams) {
    ...
    try {
        ...
        // 🛡️ Sentinel: Fetch authoritative live market data to prevent client-side price tampering
        const liveStock = await fetchStockBySymbol(symbol);
        const authPrice = liveStock.price;
        const authName = liveStock.name;
        const authSector = liveStock.sector;
        const authChangePercent = liveStock.changePercent;
