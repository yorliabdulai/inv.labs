...
export async function executeStockTrade(params: TradeParams) {
    ...
    try {
        ...
        // Security: Fetch authoritative stock data server-side
        const liveStockData = await fetchStockBySymbol(symbol);
        const serverPrice = liveStockData.price;
        const serverName = liveStockData.name;
        const serverSector = liveStockData.sector;
        const serverChangePercent = liveStockData.changePercent;
