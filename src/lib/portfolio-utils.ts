...
export function generatePortfolioHistory(
    ...
    // ⚡ Bolt: Parse dates to timestamps once and filter invalid dates
    const txsWithTime = transactions.map(tx => ({
        tx,
        timestamp: Date.parse(tx.date) // Much faster than new Date(str).getTime()
    })).filter(item => !isNaN(item.timestamp));

    txsWithTime.sort((a, b) => a.timestamp - b.timestamp);
    ...
    if (period === '1D') { days = 1; points = 24; }
        ...
        const firstTxTime = txsWithTime.length > 0 ? txsWithTime[0].timestamp : nowTime;
        const diffDays = Math.ceil((nowTime - firstTxTime) / (1000 * 60 * 60 * 24));
    ...
    for (let i = 0; i < txsWithTime.length; i++) {
        const { tx, timestamp } = txsWithTime[i];
    ...
    // ⚡ Bolt: Forward-moving simulation loop (O(N+P) instead of O(N*P))
    ...
    for (let i = points; i >= 0; i--) {
        ...
        // Advance transactions up to time `t`
        while (currentTxIndex < txsWithTime.length && txsWithTime[currentTxIndex].timestamp <= t) {
            const { tx } = txsWithTime[currentTxIndex];
