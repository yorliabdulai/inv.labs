...
export function generatePortfolioHistory(
    ...
    // ⚡ Bolt: Pre-parse timestamps outside the loop and filter invalid dates.
    const parsedTx = [...transactions]
        .map(tx => ({ ...tx, timestamp: Date.parse(tx.date) }))
    ...
    // ⚡ Bolt: Use a single forward-moving index and running state (O(N+P) vs O(N*P))
