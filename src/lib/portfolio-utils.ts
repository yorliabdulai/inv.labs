...
export function generatePortfolioHistory(
    ...
    // ⚡ Bolt: Pre-parse dates into numeric timestamps to avoid O(N*P) redundant instantiations
    const parsedTx = transactions.map(tx => ({
        ...tx,
        timestamp: Date.parse(tx.date)
    })).filter(tx => !isNaN(tx.timestamp)).sort((a, b) => a.timestamp - b.timestamp);
    ...
    if (period === '1D') { days = 1; points = 24; }
        ...
        const firstTxTime = parsedTx.length > 0 ? parsedTx[0].timestamp : now;
        const diffDays = Math.ceil((now - firstTxTime) / (1000 * 60 * 60 * 24));
    ...
    const periodStartTime = now - totalMs;
    ...
    for (let i = points; i >= 0; i--) {
        ...
        // ⚡ Bolt: Advance transactions up to time `t` rather than replaying from start
        // Changes O(P*N) to O(P+N) time complexity.
        while (currentTxIndex < parsedTx.length) {
        ...
        holdings.forEach((qty, sym) => {
            ...
            if (!fp || currentPrice === undefined) {
                ...
                const timeRatio = Math.max(0, Math.min(1, (t - fp.time) / (now - fp.time)));
